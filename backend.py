...
def insert(name,address,phone_number,no_of_days,room_type,total):
    ...
    conn=sqlite3.connect("hotel1.db")
    cur=conn.cursor()
    cur.execute("INSERT INTO hotel1 VALUES (NULL,?,?,?,?,?,?)",(name,address,phone_number,no_of_days,room_type,calculation(no_of_days,room_type)))
    conn.commit()
    conn.close()
    view()
...
def search(name="",address="",phone_number="",room_type="",no_of_days="",total=""):
    conn=sqlite3.connect("hotel1.db")
    cur=conn.cursor()
    cur.execute("SELECT * FROM hotel1 WHERE name=? and address=? and phone_number=? and room_type=? or  no_of_days=? or  total=?", (name,address,phone_number,room_type,no_of_days,total))
    rows=cur.fetchall()
    conn.close()
    return rows
...
def update(id,name,address,phone_number,room_type,no_of_days,total):
    ...
    conn=sqlite3.connect("hotel1.db")
    cur=conn.cursor()
    cur.execute("UPDATE hotel1 SET name=?, address=?, phone_number=?, room_type=? ,no_of_days=? , total=? WHERE id=?",(name,address,phone_number,room_type,no_of_days,calculation(no_of_days,room_type),id))
    conn.commit()
    conn.close()
